...
Matrix positionalEncoding(int n, int d) {
    Matrix result(n, d);

    for (int pos=0; pos<n; pos++) {
        if (d%2 == 0) {
            for (int k=0; k<d/2; k++) {
                double tmp = pow(10000, 2.0*k / d);
                result[pos][2*k] = sin(pos/tmp);
                result[pos][2*k+1] = cos(pos/tmp);
            }
        }
        else {
            ...
            double tmp = pow(10000, 2.0 * ((d-1)/2) / d);
            result[pos][d-1] = sin(pos/tmp);
        }
    }
...
private:
    ...
    // 부가 함수들
    vector<string> textToChars(const string& text);
    map<pair<string, string>, int> getPairFreq(const vector<string>& tokens);
    vector<string> applyMerag(const vector<string>& tokens, const pair<string, string>& best);
}

// 부가 함수들
vector<string> BPETokenizer::textToChars(const string& text) {
    vector<string> chars;
    for (char c : text) {
...
map<pair<string, string>, int> BPETokenizer::getPairFreq(const vector<string>& tokens) {
...
vector<string> BPETokenizer::applyMerag(const vector<string>& tokens, const pair<string, string>& best) {
    vector<string> result;
    int i=0;
    while(i<tokens.size()) {
        if (i+1 < tokens.size() && tokens[i] == best.first && tokens[i+1] == best.second) {
            result.push_back(best.first + best.second);
            i += 2;
        }
        else {
...
// 학습
void BPETokenizer::train(const string& text, int vocab_size) {
